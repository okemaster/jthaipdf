...
class ThaiDisplayUtilsTest {
	...
	class PassThrough {
		...
		@ValueSource(strings = { "", "Hello, world!", "ABC 123", "ก", "กา", "ขนม",
	...
	class UpTailWithUpperLevel1 {
		...
		static Stream<Arguments> cases() {
			return Stream.of(
					rule("ป + MAI_HAN_AKAT", s(PO_PLA, MAI_HAN_AKAT),
							s(PO_PLA, MAI_HAN_AKAT_LEFT_SHIFT)),
					rule("ฝ + SARA_I", s(FO_FA, SARA_I), s(FO_FA, SARA_I_LEFT_SHIFT)),
					rule("ฟ + SARA_Ii", s(FO_FAN, SARA_Ii),
							s(FO_FAN, SARA_Ii_LEFT_SHIFT)),
					rule("ฬ + SARA_Ue", s(LO_CHULA, SARA_Ue),
							s(LO_CHULA, SARA_Ue_LEFT_SHIFT)),
					rule("ป + SARA_Uee", s(PO_PLA, SARA_Uee),
							s(PO_PLA, SARA_Uee_LEFT_SHIFT)),
					rule("ป + MAI_TAI_KHU", s(PO_PLA, MAI_TAI_KHU),
							s(PO_PLA, MAI_TAI_KHU_LEFT_SHIFT)),
					// NIKHAHIT only appears standalone after SARA_AM explosion, but the
					// rule is the same.
					rule("ป + NIKHAHIT", s(PO_PLA, NIKHAHIT),
							s(PO_PLA, NIKHAHIT_LEFT_SHIFT)));
		}
	...
	class UpperLevel2OverRegularBase {
		...
		static Stream<Arguments> cases() {
			return Stream.of(rule("ก + MAI_EK", s('ก', MAI_EK), s('ก', MAI_EK_DOWN)),
					rule("ก + MAI_THO", s('ก', MAI_THO), s('ก', MAI_THO_DOWN)),
					rule("ก + MAI_TRI", s('ก', MAI_TRI), s('ก', MAI_TRI_DOWN)),
					rule("ก + MAI_CHATTAWA", s('ก', MAI_CHATTAWA),
							s('ก', MAI_CHATTAWA_DOWN)),
					rule("ก + THANTHAKHAT", s('ก', THANTHAKHAT),
							s('ก', THANTHAKHAT_DOWN)));
		}
	...
	class UpperLevel2OverUpTail {
		...
		static Stream<Arguments> cases() {
			return Stream.of(
					rule("ป + MAI_EK", s(PO_PLA, MAI_EK),
							s(PO_PLA, MAI_EK_PULL_DOWN_AND_LEFT_SHIFT)),
					rule("ป + MAI_THO", s(PO_PLA, MAI_THO),
							s(PO_PLA, MAI_THO_PULL_DOWN_AND_LEFT_SHIFT)),
					rule("ป + MAI_TRI", s(PO_PLA, MAI_TRI),
							s(PO_PLA, MAI_TRI_PULL_DOWN_AND_LEFT_SHIFT)),
					rule("ป + MAI_CHATTAWA", s(PO_PLA, MAI_CHATTAWA),
							s(PO_PLA, MAI_CHATTAWA_PULL_DOWN_AND_LEFT_SHIFT)),
					rule("ป + THANTHAKHAT", s(PO_PLA, THANTHAKHAT),
							s(PO_PLA, THANTHAKHAT_PULL_DOWN_AND_LEFT_SHIFT)));
		}
	...
	class DownTailWithLowerLevel {
		...
		static Stream<Arguments> cutTailCases() {
			return Stream.of(
					...
					rule("ฐ + SARA_UU", s(THO_THAN, SARA_UU),
							s(THO_THAN_CUT_TAIL, SARA_UU)),
					rule("ฐ + PHINTHU", s(THO_THAN, PHINTHU),
							s(THO_THAN_CUT_TAIL, PHINTHU)),
					rule("ญ + SARA_U", s(YO_YING, SARA_U), s(YO_YING_CUT_TAIL, SARA_U)),
					rule("ญ + SARA_UU", s(YO_YING, SARA_UU),
							s(YO_YING_CUT_TAIL, SARA_UU)));
		}

		// ฐ and ญ have a dedicated cut-tail glyph, so the base is rewritten and the vowel
		// left alone.
		@ParameterizedTest(name = "[{index}] {0} cuts the tail off the base")
		@MethodSource("cutTailCases")
		void cuts_tail_when_base_has_dedicated_glyph(String name, String input,
				String expected) {
			assertEquals(expected, toDisplayString(input));
		}
		...
		// The remaining down-tail consonants have no cut-tail variant, so we pull the
		// vowel down instead.
		@ParameterizedTest(name = "[{index}] {0} pulls the vowel down")
		@MethodSource("pullDownCases")
		void pulls_down_when_base_has_no_cut_tail_glyph(String name, String input,
				String expected) {
			assertEquals(expected, toDisplayString(input));
		}
	}
	...
	class SaraAmExplosion {
		...
		// ปำ → ป + NIKHAHIT_LEFT_SHIFT + SARA_AA — NIKHAHIT shifted because base is
		// up-tail.
		@Test
		...
		// ก่ำ → upper-level-2 sits between base and SARA_AM; NIKHAHIT is inserted before
		// the level-2 mark
		...
		// ป่ำ → up-tail + upper-level-2 + SARA_AM. NIKHAHIT is inserted, gets shifted
		// left over ป,
	...
	class RealWords {
		...
		// ใหญ่ (big) — ญ here has no following lower vowel, so it stays whole; MAI_EK
		// over ญ pulls down.
		@Test
		void yai() {
	...
	class CharHelpers {
		...
		void isLowerLevel_is_true_only_for_lower_vowels() {
			assertAll(() -> assertTrue(isLowerLevel(SARA_U)),
		...
		void shiftLeft_maps_each_known_mark_and_passes_others_through() {
			assertAll(() -> assertEquals(MAI_EK_LEFT_SHIFT, shiftLeft(MAI_EK)),
		...
		void pullDownAndShiftLeft_maps_each_known_mark_and_passes_others_through() {
			assertAll(
					() -> assertEquals(MAI_EK_PULL_DOWN_AND_LEFT_SHIFT,
							pullDownAndShiftLeft(MAI_EK)),
					() -> assertEquals(MAI_THO_PULL_DOWN_AND_LEFT_SHIFT,
							pullDownAndShiftLeft(MAI_THO)),
					() -> assertEquals(MAI_TRI_PULL_DOWN_AND_LEFT_SHIFT,
							pullDownAndShiftLeft(MAI_TRI)),
					() -> assertEquals(MAI_CHATTAWA_PULL_DOWN_AND_LEFT_SHIFT,
							pullDownAndShiftLeft(MAI_CHATTAWA)),
					() -> assertEquals(MAI_HAN_AKAT_LEFT_SHIFT,
							pullDownAndShiftLeft(MAI_HAN_AKAT)),
					() -> assertEquals(THANTHAKHAT_PULL_DOWN_AND_LEFT_SHIFT,
							pullDownAndShiftLeft(THANTHAKHAT)),
